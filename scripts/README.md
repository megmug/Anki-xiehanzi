...
These scripts are the current source-of-truth build pipeline for this fork's hanzi APKG.
...
- `build_cc_cedict_master_db.py`: build the compact CC-CEDICT master lexicon from the pinned snapshot in `deck_inputs/cc-cedict/`.
- `enrich_hanzi_db.py`: attach hanzi study targets from the deck input word lists to the master lexicon.
- `meaning_html.py`: render hanzi-style Meaning HTML from structured word and form data.
